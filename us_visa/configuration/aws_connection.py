...
class S3Client:
    ...
    def __init__(self, region_name="eu-central-1"):
        ...
        logging.info(f"AWS_ACCESS_KEY_ID_ENV_KEY: {__access_key_id}")
        logging.info(f"AWS_SECRET_ACCESS_KEY_ENV_KEY: {__secret_access_key}")

        if not __access_key_id or not __secret_access_key:
            raise Exception(f"Missing AWS credentials. "
                            f"AWS_ACCESS_KEY_ID_ENV_KEY: {__access_key_id}, "
                            f"AWS_SECRET_ACCESS_KEY_ENV_KEY: {__secret_access_key}")
